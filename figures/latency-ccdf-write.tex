...
\addplot [mark=x,color=TUMDarkerBlue, thick,mark repeat*=11] table[col sep=comma,x=latency,y expr=1-\thisrow{cdf}] {data/libaio-write-cdf.csv};
\addplot [mark=triangle*,color=TUMGreen, thick,mark repeat*=7] table[col sep=comma,x=latency,y expr=1-\thisrow{cdf}] {data/psync-write-cdf.csv};
\addplot [mark=+,color=TUMOrange, thick,mark repeat*=20] table[col sep=comma,x=latency,y expr=1-\thisrow{cdf}] {data/vroom-write-cdf.csv};
\addplot [mark=square*,color=TUMRed, thick,mark repeat*=11] table[col sep=comma,x=latency,y expr=1-\thisrow{cdf}] {data/spdk-write-cdf.csv};
...
\node[anchor = south east,yshift=-0.425em] at (axis cs: 100.5,0.0001) { 99.99th percentile};
\node[anchor = south east,yshift=-0.425em] at (axis cs: 100.5,0.00001) { 99.999th percentile};
\end{pgfonlayer}
\end{axis}
\end{tikzpicture}
\end{document}
